...
const [EnteredTask,setEnteredTask]= useState(props.route.params.task);
const [TaskItem,setTaskItem]=useState([]);
const [date, setDate] = useState(new Date());
const [time, setTime] = useState(new Date());
const [mode, setMode] = useState('date');
const [show, setShow] = useState(false);
const [dateValue, setDateValue] = useState(props.route.params.Date);
const [timeValue, setTimeValue] = useState(props.route.params.Time);
...
const onPressHandler =()=> {
    ...
    navigation.navigate('ViewList', {taskData: TaskItem}) 
}
...
const addTaskHandler = () => {
    if (todos && !todos.includes(EnteredTask) && EnteredTask!='') {
